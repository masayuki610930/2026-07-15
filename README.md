...
- 集計画面（`dashboard.html?key=…`）をプロジェクタに映しておくと、回答が集まるたびに棒グラフレースが動き、新着トーストや節目（10/25/50/75/100人・16タイプコンプリート）の紙吹雪、「🔥 ◯◯が日本平均のx倍！」ハイライトで盛り上がります。回答数は「回答: n人」の実数のみで、参加予定人数（分母）は表示しません。
